...
up_mid_down = pd.read_csv(sp_dir + '/Upstream/upstream_data_pipeline_sp/Postprocessed_outputs_2/downstream_postprocessed_scenarios.csv')
...
up_mid_down_base = pd.read_csv(sp_dir + '/Upstream/upstream_data_pipeline_sp/Postprocessed_outputs_2/downstream_postprocessed.csv')
...
scenario_rh_def = prep_for_webtool(up_mid_down_base[(up_mid_down_base['gwp']==20) 
    & up_mid_down_base['Field_name'].isin(['Kern River','Frade', 'Syncrude Sweet Premium'])])
...
scenario = pd.concat([scenario,scenario_rh_def,scenario_rh_on, scenario_es_def, 
    scenario_es_on,scenario_fv_def,scenario_fv_50,scenario_fv_150])
...
def toggle_stage(x):
    if x in(['Liquefied Natural Gas', 'Liquefied Natural Gas', 'Petroleum Coke Combustion']):
        return 'Downstream'
    elif x in(['Carbon Capture and Storage', 'Renewable Electricity','Renewable Electricity',
            'Flare Efficiency','Venting + Fugitive Leakage', 'Solar Steam', 'Energy to Pump Water', 'Flare Volume']):
        return 'Upstream'
...
scenario.to_csv(sp_dir + '/Upstream/upstream_data_pipeline_sp/Postprocessed_outputs_2/sliders.csv')
scenario.to_csv('/Users/rwang/Documents/oci/basedata/sliders.csv')
